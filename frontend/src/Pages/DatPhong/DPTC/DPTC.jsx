...
const DPTC = ({paymentType}) => {
    ...
    useEffect(()=>{
        ttHD&&
        setText(`Mã hoá đơn: ${ttHD.hoaDonID} \n
                            Khách hàng Họ tên: ${ttHD.hoTenKH}, Số điện thoại: ${user.soDienThoai}, Email: ${user.email}\n
                            Phòng Loại phòng: ${additionalInfo.loaiPhong}, Ngày nhận phòng: ${ttHD.ngayNhanPhong}, Ngày trả phòng: ${ttHD.ngayTraPhong}, Chi phí 1 ngày :${additionalInfo.giaPhong}\n
    ...
    const user=JSON.parse(localStorage.getItem('user'))||{maNguoiDung:"ND10000000000001",email:"user1@example.com",soDienThoai:"0123456789",tenDangNhap:"user1"};
    const additionalInfo=JSON.parse(localStorage.getItem("datPhongInfor"));
    return (
        <div id="dptc" style={{marginTop:"3vh"}}>
            ...
            {ttHD&&(
                <div id="bill">
                    <i>Mã hoá đơn: {ttHD.hoaDonID}</i>
                    <h3>Khách hàng</h3>
                    <div id="customer">
                        <p>Tên đầy đủ: <b>{ttHD.hoTenKH}</b></p>
                    ...
                    <table id="roomBill" style={{marginTop: "5vh"}} >
                        ...
                        <tr>
                            ...
                            <td>{(Number(ttHD.chiPhiDuTinh) / Number(ttHD.phong.giaPhong))}</td>
                            <td>{additionalInfo.giaPhong.toLocaleString("vn-VN")} VNĐ</td>
                        </tr>
                    </table>
